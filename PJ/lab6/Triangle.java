 ...
 */
	...
	public boolean isItInside(int x, int y) {
		double AB, BC, AC, PA, PB, PC;
		double alfa, beta, gamma, angle;
		double radToDeg = 180/Math.PI;
		
		AB = (A.getX() - B.getX())*(A.getX() - B.getX()) + (A.getY() - B.getY())*(A.getY() - B.getY());
		BC = (B.getX() - C.getX())*(B.getX() - C.getX()) + (B.getY() - C.getY())*(B.getY() - C.getY());
		AC = (A.getX() - C.getX())*(A.getX() - C.getX()) + (A.getY() - C.getY())*(A.getY() - C.getY());
		PA = (x - A.getX())*(x - A.getX()) + (y - A.getY())*(y - A.getY());
		PB = (x - B.getX())*(x - B.getX()) + (y - B.getY())*(y - B.getY());
		PC = (x - C.getX())*(x - C.getX()) + (y - C.getY())*(y - C.getY());
		
		alfa = Math.acos( (PA+PB-AB)/(2*Math.sqrt(PA)*Math.sqrt(PB))) * radToDeg;
		beta = Math.acos( (PB+PC-BC)/(2*Math.sqrt(PB)*Math.sqrt(PC))) * radToDeg;
		gamma = Math.acos( (PA+PC-AC)/(2*Math.sqrt(PA)*Math.sqrt(PC))) * radToDeg;
		
		angle = Math.round(alfa+beta+gamma);
